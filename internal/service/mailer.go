...
// Mailer abstrae el envío de emails. La interfaz vive en service para que
// AuthService no dependa de una implementación concreta.
type Mailer interface {
	SendVerificacion(ctx context.Context, email, nombre, link string) error
	SendPasswordReset(ctx context.Context, email, nombre, link string) error
}
...
func (m *smtpMailer) SendPasswordReset(_ context.Context, email, nombre, link string) error {
	msg := buildPasswordResetEmail(m.cfg.From, email, nombre, link)
	addr := net.JoinHostPort(m.cfg.Host, m.cfg.Port)
...
// buildPasswordResetEmail arma el mensaje MIME para el reseteo de contraseña.
func buildPasswordResetEmail(from, to, nombre, link string) []byte {
	nombreSeguro := html.EscapeString(nombre)
	linkSeguro := html.EscapeString(link)
	subject := "MIME-Version: 1.0\r\n" +
		"Content-Type: text/html; charset=\"UTF-8\"\r\n" +
		...
		"\r\n"
	body := "<p>Hola " + nombreSeguro + ",</p>" +
		"<p>Recibimos un pedido para restablecer tu contraseña. Hacé clic en el siguiente botón para crear una nueva:</p>" +
		"<p><a href=\"" + linkSeguro + "\" style=\"display:inline-block;padding:10px 20px;background:#2563eb;color:#ffffff;text-decoration:none;border-radius:6px;font-family:sans-serif;\">Restablecer contraseña</a></p>" +
		"<p>Si el botón no funciona, copiá y pegá este enlace en tu navegador:</p>" +
		"<p><a href=\"" + linkSeguro + "\">" + linkSeguro + "</a></p>" +
		"<p style=\"color:#6b7280;font-family:sans-serif\">El enlace vence en 30 minutos. Si no pediste este cambio, ignorá este mensaje.</p>"
	return []byte(subject + body)
}
...
func (m *logMailer) SendPasswordReset(_ context.Context, email, _, link string) error {
	slog.Info("[mailer modo dev] link de reseteo de contraseña", "email", email, "link", strings.TrimSpace(link))
	return nil
}
