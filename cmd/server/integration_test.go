...
func TestBalance_AhorroAcumulado(t *testing.T) {
	env := newTestEnv(t)
	...
	// Sin deudas no hay pasivos ni patrimonio en el balance.
	if strings.Contains(body, "Pasivos") || strings.Contains(body, "PATRIMONIO") {
		t.Errorf("balance no debería mostrar pasivos/patrimonio: %s", body)
	}
}

func TestMeses_CierreTotales(t *testing.T) {
	env := newTestEnv(t)
...
func TestMeses_CierreCalculaAhorroYRecalcularCerrado(t *testing.T) {
	env := newTestEnv(t)
	...
	// Cerrar: el ahorro acumulado queda congelado en el mes cerrado.
	rec = doReq(t, env.router, http.MethodPost, fmt.Sprintf("/api/meses/%d/cerrar", mesID), token, "", "", false)
	if rec.Code != http.StatusOK {
		t.Fatalf("cerrar: expected 200, got %d (%s)", rec.Code, rec.Body.String())
	}
	...
	json.Unmarshal(rec.Body.Bytes(), &mes)
	if mes["ahorro_acumulado"].(float64) != 100000 {
		t.Errorf("closed month persisted wrong acumulados: ahorro=%v", mes["ahorro_acumulado"])
	}
...
func TestDashboard_MesActualCerrado_MuestraMesAbierto(t *testing.T) {
	...
	// Un egreso en el mes abierto siguiente queda reflejado en el dashboard.
	createTransaction(t, env, token, "egreso", 80000, proximoPeriodo+"-10")

	// El dashboard debe reflejar el mes abierto, con el nuevo egreso.
	rec = doReq(t, env.router, http.MethodGet, "/api/dashboard", token, "", "", false)
	if rec.Code != http.StatusOK {
		t.Fatalf("dashboard: expected 200, got %d", rec.Code)
	}
	...
	if mesActual["egresos_total"].(float64) != 80000 {
		t.Errorf("los egresos del mes abierto deberían ser 80000, got %v", mesActual["egresos_total"])
	}
}
...
func TestDashboard_FeedDeTransacciones(t *testing.T) {
	env := newTestEnv(t)
	token, _ := registerJSON(t, env, "dashfeed@test.com")
	dia := time.Now().Format("2006-01-02")

	createTransaction(t, env, token, "egreso", 25000, dia)
	createTransaction(t, env, token, "ingreso", 60000, dia)
	...
	ultimos, _ := dash["ultimos_movimientos"].([]interface{})
	if len(ultimos) != 2 {
		t.Fatalf("expected 2 ultimos_movimientos (2 transacciones), got %d", len(ultimos))
	}
	for _, m := range ultimos {
		mm, _ := m.(map[string]interface{})
		if mm["origen"] != "transaccion" {
			t.Errorf("expected origen transaccion, got %v", mm["origen"])
		}
	}
	mesActual, _ := dash["mes_actual"].(map[string]interface{})
	if mesActual["egresos_total"].(float64) != 25000 || mesActual["ingresos_total"].(float64) != 60000 {
		t.Errorf("totales incorrectos: %v", mesActual)
	}
}
...
func TestDashboard_FeedFiltradoPorMes(t *testing.T) {
	...
	// Dos transacciones creadas hoy, dentro del período actual.
	createTransaction(t, env, token, "egreso", 1000, time.Now().Format("2006-01-02"))
	createTransaction(t, env, token, "ingreso", 2000, time.Now().Format("2006-01-02"))
...
func TestMigrations_UpgradeDesdeV1(t *testing.T) {
	...
	var tabla int
	// Las tablas eliminadas en 008 (deudas, costos_fijos) no deben existir tras el upgrade.
	for _, tname := range []string{"deudas", "costos_fijos"} {
		if err := db.QueryRow("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = ? AND table_name = ?", name, tname).Scan(&tabla); err != nil {
			t.Fatalf("query %s table: %v", tname, err)
		}
		if tabla != 0 {
			t.Errorf("tabla %s no debería existir tras el upgrade, encontrada %d", tname, tabla)
		}
	}
	var version int64
